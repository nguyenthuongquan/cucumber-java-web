@FB-LoginPage
Feature: F20 - Facebook Login page feature

  Background:
    Given user navigates to Facebook landing page

  @TC20-001
  Scenario Outline: TC20-001 FB Login page - verify login with incorrect credentials
...
#      | <script>alert(\"testing\")</script> | 123456   | Sai thông tin đăng nhập |
  ...
  @TC20-002
  Scenario Outline: TC20-002 FB Login page - verify login with correct credentials
    When user logins Facebook with email "<email>" and password "<password>"
