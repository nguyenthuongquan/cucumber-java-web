@FB-LoginPage
Feature: F16 - Facebook Login page feature

  Background:
    Given user navigates to Facebook landing page

  @TC16-001
  Scenario Outline: TC16-001 FB Login page - verify login with incorrect credentials
...
#      | <script>alert(\"testing\")</script> | 123456   | Sai thông tin đăng nhập |
  ...
  @TC16-002
  Scenario Outline: TC16-002 FB Login page - verify login with correct credentials
    When user logins Facebook with email "<email>" and password "<password>"
